...
export interface WeekImportLog {
  id: string;
  file_name: string;
...
interface ScorecardWeekDropZoneProps {
  children: React.ReactNode;
  weekStartDate: string; // ISO date string like "2026-01-06"
  weekLabel: string; // e.g., "1/6-1/12"
  onFileDrop: (result: CSRParseResult, fileName: string, weekStartDate: string, file: File) => void;
  className?: string;
  /** Most recent import log for this week */
  importLog?: WeekImportLog | null;
  /** Called when user wants to re-import from an existing file */
  onReimport?: (filePath: string, fileName: string, weekStartDate: string) => void;
}

export const ScorecardWeekDropZone = ({
  children,
  weekStartDate,
  weekLabel,
  onFileDrop,
  ...
  const handleDrop = useCallback(async (e: React.DragEvent) => {
    ...
    try {
      ...
      onFileDrop(result, excelFile.name, weekStartDate, excelFile);
    } catch (error: any) {
      toast({
        title: "Parse Error",
  ...
  }, [weekStartDate, onFileDrop, toast]);
  ...
  const handleReimport = useCallback(async () => {
    ...
    try {
      onReimport(importLog.report_file_path, importLog.file_name, weekStartDate);
  ...
  }, [importLog?.report_file_path, importLog?.file_name, weekStartDate, onReimport, toast]);
  ...
  return (
    <>
      ...
      >
        ...
        {/* Import log indicator - positioned at top-left */}
      ...
      <Dialog open={showDetailsDialog} onOpenChange={setShowDetailsDialog}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Weekly Import Details</DialogTitle>
          </DialogHeader>
          {importLog && (
