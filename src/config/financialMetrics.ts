...
export const KTRV_METRICS: FinancialMetric[] = GMC_CHEVROLET_METRICS.filter(
  ...
  if (
    ["total_sales", "gp_net", "gp_percent", "sales_expense", "semi_fixed_expense", "total_fixed_expense"].includes(
      metric.key,
    )
  ) {
    return { ...metric, hasSubMetrics: true };
...
export const getMetricsForBrand = (brand: string | null): FinancialMetric[] => {
  ...
  if (
    brand?.toLowerCase().includes("stellantis") ||
    brand?.toLowerCase().includes("chrysler") ||
    brand?.toLowerCase().includes("jeep") ||
    brand?.toLowerCase().includes("dodge") ||
    brand?.toLowerCase().includes("ram")
  ) {
